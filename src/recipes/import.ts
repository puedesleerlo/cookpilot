...
import { normalizeRecipe, type NormalizeInput } from '@/llm/stages/normalize';
...
/** Import pasted recipe text through stage L3, falling back to the deterministic parser. */
export const importRecipeText = async (input: NormalizeInput): Promise<ImportResult> => {
  const outcome = await normalizeRecipe(input);
  if (!outcome.ok) return { ok: false, reason: outcome.reason };
  const recipes = [outcome.recipe];
  return {
    ok: true,
    pack: RecipePackSchema.parse({
      packId: makeId('pack', 'imported', outcome.recipe.id),
      version: '1.0.0',
      contentHash: contentHash(recipes),
      name: outcome.recipe.title,
      description: outcome.source === 'fallback' ? 'Read without the model, using the verb rules.' : '',
      locale: 'en-US',
      provenance: 'imported',
      recipes,
    }),
